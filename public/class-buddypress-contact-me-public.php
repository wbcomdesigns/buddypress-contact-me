...
class BuddyPress_Contact_Me_Public {
	...
	public function bp_contact_me_email( $get_contact_id, $bp_display_user_id ) {
		...
		// Include sender's email if the option is enabled. Matching the
		// isset() && 'yes' === pattern used by the other three toggles
		// — the previous array_key_exists check was fooled by legacy
		// rows where the key existed with a falsy/non-'yes' value.
		if ( isset( $bcm_general_setting['bcm_allow_sender_copy_email'] ) && 'yes' === $bcm_general_setting['bcm_allow_sender_copy_email'] ) {
	...
	public function bp_contact_me_form_submitted() {
		...
		if (
			...
			// Defence-in-depth: re-verify the sender is allowed to send
			// messages, matching the gate in bp_contact_me_tab(). A
			// crafted POST could otherwise skip the tab render gate.
			// Only logged-in members are subject to the role filter;
			// non-logged-in senders are validated via captcha below.
			if ( $bp_sender_user_id ) {
				$bcm_settings = get_option( 'bcm_admin_general_setting', array() );
				$who_contact  = isset( $bcm_settings['bcm_who_contact'] ) && is_array( $bcm_settings['bcm_who_contact'] )
					? $bcm_settings['bcm_who_contact']
					: array();
				if ( ! empty( $who_contact ) ) {
					$sender      = get_userdata( $bp_sender_user_id );
					$sender_roles = $sender ? (array) $sender->roles : array();
					// Allow admins through unconditionally. Otherwise at
					// least one of the sender's roles must be listed.
					if ( ! in_array( 'administrator', $sender_roles, true )
						&& empty( array_intersect( $sender_roles, $who_contact ) ) ) {
						return false;
					}
				}
			}
